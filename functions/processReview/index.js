...
const error = (context) => {
  context.res = {
    ...
    body: JSON.stringify({
      ...
      message: "Please provide a properly formatted request",
    }),
  };
};
...
const toOrgFilename = (review) => {
  return `${moment().format("YYYYMMDD")}_${review.firstName}_${
    review.lastName
  }.org`;
};
...
const toMarkup = (review) => {
  ...
  review.achievements.forEach(
    (achievement) => (markup += `  * ${achievement}\n`)
  );
  markup += "\n";
  markup += "\n";
  markup += "== Behaviour ==\n";
  ...
  review.opportunities.forEach(
    (opportunity) => (markup += `  * ${opportunity}\n`)
  );
